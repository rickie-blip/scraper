...
export default function TrackerPage() {
  ...
  const [scrapeAllSummary, setScrapeAllSummary] = useState(null);
  ...
  const productUrlWarning = productForm.product_url && !productForm.product_url.includes("/products/");

  function summarizeScrapeResults(results = []) {
  ...
  useEffect(() => {
    ...
    if (!competitorNames.includes(liveForm.base_competitor)) {
      setLiveForm((s) => ({ ...s, base_competitor: competitorNames[0] }));
    }
  }, [competitorNames, liveForm.base_competitor]);
  ...
  async function onAddProduct(e) {
    ...
    setError("");
    try {
      if (productUrlWarning) {
        setError("Product URL should include /products/ for best Shopify scraping results.");
        return;
      }
  ...
  async function onScrapeProduct(productId) {
    ...
    try {
      ...
      if (res?.entry?.price != null) {
        setScrapeProductMessage(`Scrape succeeded. Latest price: ${formatPrice(res.entry.price)}.`);
      }
    } catch (err) {
      setError(err.message);
    }
  ...
  async function onScrapeAll() {
    ...
    try {
      ...
      setScrapeAllSummary({
        ...summarizeScrapeResults(res?.results || []),
  ...
  function exportProducts() {
    ...
    downloadCsv(
      "tracked-products.csv",
      products.map((p) => ({
        product_name: p.product_name,
        category: p.category,
        competitor: p.competitor_name,
        latest_price: p.latest_price,
        latest_collected_at: p.latest_collected_at,
        product_url: p.product_url,
      }))
    );
  }

  function exportLiveCompare() {
    if (!liveResult?.matches?.length) return;
    downloadCsv(
      "live-compare.csv",
      liveResult.matches.map((row) => ({
        ...
        price: row.price,
        delta_vs_vivo: row.delta_vs_vivo,
        delta_pct_vs_vivo: row.delta_pct_vs_vivo,
  ...
  function exportHistory() {
    ...
    downloadCsv(
      `price-history-${historyProductName || "product"}.csv`,
      history.map((row) => ({
  ...
  function exportUpdates() {
    if (!sortedUpdates.length) return;
    downloadCsv(
      "latest-updates.csv",
      sortedUpdates.map((row) => ({
        product_name: row.product_name,
        price: row.price,
        collected_at: row.collected_at,
      }))
  ...
  return (
    <div className="panel">
      ...
      <div className="metrics">
        ...
        {scrapeAllSummary && (
          <div className="metric-card">
            <span>Last Bulk Scrape</span>
            <strong>
              {scrapeAllSummary.ok} OK / {scrapeAllSummary.failed} Failed
            </strong>
            <span className="muted">Total: {scrapeAllSummary.total}</span>
          </div>
        )}
      </div>
        ...
        <div className="card-block">
          ...
          {productUrlWarning && (
            <div className="alert alert-warning mt-2">
              Shopify URLs usually include <code>/products/</code>. Non-Shopify pages may fail to scrape.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="table-responsive mt-3">
        <div className="table-actions">
          ...
          <button className="btn btn-outline-primary btn-sm" onClick={exportProducts} disabled={!products.length}>
            Export CSV
          </button>
        </div>
      ...
      <div className="split mt-3">
        <div className="card-block">
          <div className="table-actions">
            ...
            <button className="btn btn-outline-primary btn-sm" onClick={exportLiveCompare} disabled={!liveResult?.matches?.length}>
              Export CSV
            </button>
          </div>
        ...
        <div className="card-block">
          <div className="table-actions">
            ...
            <button className="btn btn-outline-primary btn-sm" onClick={exportComparison} disabled={!comparison?.rows?.length}>
              Export CSV
            </button>
          </div>
      ...
      <div className="split mt-3">
        <div className="card-block">
          <div className="table-actions">
            <h5>Price History</h5>
            <button className="btn btn-outline-primary btn-sm" onClick={exportHistory} disabled={!history.length}>
              Export CSV
            </button>
          </div>
        ...
        <div className="card-block">
          <div className="table-actions">
            ...
            <button className="btn btn-outline-primary btn-sm" onClick={exportUpdates} disabled={!sortedUpdates.length}>
              Export CSV
            </button>
          </div>
